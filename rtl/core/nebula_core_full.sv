 ...
 *
 * CORREÇÕES APLICADAS:
 * 1. frontend_exc_cause mudado de logic [5:0] para tipo alinhado com
 *    nebula_frontend_rvc (que agora emite logic [5:0]).
 * 2. Novos sinais de escrita do PTW (ptw_mem_we, ptw_mem_wdata)
 *    conectados — necessários para o writeback dos bits A/D.
...
module nebula_core_full #(
    ...
    // FIX 2: novos sinais de write para PTW A/D writeback
    ...
    // D-Cache
    logic                    dcache_req, dcache_we, dcache_is_amo;
    ...
    logic                    itlb_access_fault;
    logic                    dtlb_req, dtlb_hit, dtlb_page_fault;
    logic                    dtlb_is_store, dtlb_access_fault;
    ...
    logic                    ptw_ready, ptw_resp_valid, ptw_page_fault;
    logic                    ptw_access_fault, ptw_is_for_itlb;
    ...
    logic                    ptw_resp_r, ptw_resp_w, ptw_resp_x;
    logic                    ptw_resp_u, ptw_resp_g, ptw_resp_a, ptw_resp_d;
    ...
    // Trap
    logic                    trap_enter, trap_is_interrupt, mret_exec, sret_exec;
    logic                    interrupt_pending;
    logic [XLEN-1:0]         trap_pc, trap_cause, trap_value;
    logic [XLEN-1:0]         trap_vector, return_pc, interrupt_cause;
    ...
    logic                    fence_i_valid, dcache_flush, dcache_flush_done;
    ...
    // =========================================================================
    branch_predictor #(
        .VADDR_WIDTH(VADDR_WIDTH), .BTB_ENTRIES(256),
        .BHT_ENTRIES(1024), .RAS_DEPTH(8), .GHR_LEN(10)
    ) u_bp (
        .clk, .rst_n,
        .pc(fetch_pc), .predict_valid(icache_req),
        .prediction(bp_prediction),
        .update_valid(bp_update.valid), .update(bp_update)
    );
    ...
    // =========================================================================
    icache_l1 #(
        .PADDR_WIDTH(PADDR_WIDTH), .LINE_SIZE(L1_LINE_SIZE),
        .NUM_WAYS(L1_WAYS), .NUM_SETS(L1_SETS)
    ) u_icache (
        .clk, .rst_n,
        .req_valid(icache_req), .req_addr(icache_addr),
        .req_ready(icache_ready),
        .resp_valid(icache_resp_valid), .resp_data(icache_resp_data),
        .resp_error(icache_resp_error),
        .invalidate_all(fence_i_valid),
        .invalidate_addr_valid(1'b0), .invalidate_addr('0),
        .mem_req(imem_req), .mem_addr(imem_addr),
        .mem_ack(imem_ack), .mem_data(imem_data), .mem_error(imem_error)
    );
    ...
    // =========================================================================
    dcache_l1 #(
        .PADDR_WIDTH(PADDR_WIDTH), .XLEN(XLEN),
        .LINE_SIZE(L1_LINE_SIZE), .NUM_WAYS(L1_WAYS), .NUM_SETS(L1_SETS)
    ) u_dcache (
        .clk, .rst_n,
        .req_valid(dcache_req), .req_addr(dcache_addr),
        .req_wdata(dcache_wdata), .req_wstrb(dcache_wstrb),
        .req_we(dcache_we), .req_is_amo(dcache_is_amo),
        .req_amo_op(dcache_amo_op), .req_ready(dcache_ready),
        .resp_valid(dcache_resp_valid), .resp_rdata(dcache_resp_data),
        .resp_error(dcache_resp_error),
        .invalidate_all(1'b0), .flush_all(dcache_flush),
        .flush_addr_valid(1'b0), .flush_addr('0),
    ...
    // FPU
    // =========================================================================
    fpu_ieee754 #(.FLEN(FLEN)) u_fpu (
        .clk, .rst_n,
        .req_valid(fpu_req), .req_ready(fpu_ready),
        .op(fpu_op), .rs1(fpu_rs1), .rs2(fpu_rs2), .rs3(fpu_rs3),
        .int_rs1(fpu_int_rs1), .rm(fpu_rm_final), .is_single(fpu_is_single),
        .resp_valid(fpu_resp_valid), .result(fpu_result),
        .int_result(fpu_int_result), .fflags(fpu_fflags)
    );
    ...
    mdu_rv64 #(.XLEN(XLEN)) u_mdu (
        .clk, .rst_n,
        .req_valid(mdu_req), .req_ready(mdu_ready),
        .rs1_data(mdu_rs1), .rs2_data(mdu_rs2),
        .funct3(mdu_funct3), .is_word_op(mdu_is_word),
        .resp_valid(mdu_resp_valid), .result(mdu_result)
    );
    ...
    csr_unit #(.XLEN(XLEN), .HART_ID(HART_ID)) u_csr (
        .clk, .rst_n,
        .csr_req_valid(csr_req), .csr_addr, .csr_wdata,
        .csr_op, .current_priv,
        .csr_rdata, .csr_access_fault(csr_fault),
        .timer_irq_in(timer_irq), .external_irq_in(external_irq),
        .software_irq_in(software_irq),
        .trap_enter, .trap_is_interrupt, .trap_pc, .trap_cause, .trap_value,
        .trap_from_priv(current_priv),
        .mret_execute(mret_exec), .sret_execute(sret_exec),
        .trap_vector, .return_pc, .return_priv,
        .interrupt_pending, .interrupt_cause,
        .current_priv_out(current_priv),
        .csr_satp, .csr_mstatus,
        .mstatus_mie(), .mstatus_sie(), .mstatus_tvm(), .mstatus_tsr(),
        .mstatus_mprv(), .mstatus_mpp(), .mstatus_spp(),
        .cycle_count(cycle_counter), .time_count(time_counter),
        .instret_count(instret_counter),
        .fflags_in(fpu_fflags), .fflags_we(fpu_resp_valid), .frm_out(csr_frm)
    );

    // =========================================================================
    // ITLB
    // =========================================================================
    tlb_sv39 #(.TLB_ENTRIES(TLB_ENTRIES)) u_itlb (
        .clk, .rst_n,
        .lookup_valid(itlb_req), .lookup_vpn(itlb_vpn),
        .lookup_asid(csr_satp[59:44]),
        .lookup_priv(current_priv), .lookup_is_store(1'b0),
        .lookup_is_exec(1'b1),
        .mstatus_sum(csr_mstatus[18]), .mstatus_mxr(csr_mstatus[19]),
        .lookup_hit(itlb_hit), .lookup_ppn(itlb_ppn),
        .lookup_page_fault(itlb_page_fault),
        .access_fault(itlb_access_fault),
        .need_set_a(itlb_need_a), .need_set_d(itlb_need_d),
        .fault_vpn(itlb_fault_vpn),
        .insert_valid(ptw_resp_valid && ptw_is_for_itlb),
        .insert_vpn(ptw_resp_vpn), .insert_ppn(ptw_resp_ppn),
        .insert_asid(ptw_resp_asid), .insert_page_size(ptw_resp_page_size),
        .insert_r(ptw_resp_r), .insert_w(ptw_resp_w), .insert_x(ptw_resp_x),
        .insert_u(ptw_resp_u), .insert_g(ptw_resp_g),
        .insert_a(ptw_resp_a), .insert_d(ptw_resp_d),
        ...
        .invalidate_by_both(sfence_valid && !sfence_all &&
                            (sfence_vpn != '0) && (sfence_asid != '0)),
    ...
    // DTLB
    // =========================================================================
    tlb_sv39 #(.TLB_ENTRIES(TLB_ENTRIES)) u_dtlb (
        .clk, .rst_n,
        .lookup_valid(dtlb_req), .lookup_vpn(dtlb_vpn),
        .lookup_asid(csr_satp[59:44]),
        .lookup_priv(current_priv), .lookup_is_store(dtlb_is_store),
        .lookup_is_exec(1'b0),
        .mstatus_sum(csr_mstatus[18]), .mstatus_mxr(csr_mstatus[19]),
        .lookup_hit(dtlb_hit), .lookup_ppn(dtlb_ppn),
        .lookup_page_fault(dtlb_page_fault),
        .access_fault(dtlb_access_fault),
        .need_set_a(dtlb_need_a), .need_set_d(dtlb_need_d),
        .fault_vpn(dtlb_fault_vpn),
        .insert_valid(ptw_resp_valid && !ptw_is_for_itlb),
        .insert_vpn(ptw_resp_vpn), .insert_ppn(ptw_resp_ppn),
        .insert_asid(ptw_resp_asid), .insert_page_size(ptw_resp_page_size),
        .insert_r(ptw_resp_r), .insert_w(ptw_resp_w), .insert_x(ptw_resp_x),
    ...
    // =========================================================================
    // PTW — FIX 2: conectar novos sinais de write
    // =========================================================================
    ptw_sv39 u_ptw (
        .clk, .rst_n,
        ...
        .ptw_req_is_exec(ptw_req_itlb && !ptw_req_dtlb),
        .ptw_req_for_itlb(ptw_is_for_itlb),
        .ptw_req_ready(ptw_ready),
        .satp(csr_satp),
        // Memória — leitura e escrita (A/D writeback)
        .ptw_mem_req,
        .ptw_mem_addr,
        .ptw_mem_we,
        .ptw_mem_wdata,
        ...
        // Resposta
        .ptw_resp_valid, .ptw_resp_vpn, .ptw_resp_ppn,
        .ptw_resp_asid, .ptw_resp_page_size,
        .ptw_resp_page_fault(ptw_page_fault),
        .ptw_resp_access_fault(ptw_access_fault),
        .ptw_resp_for_itlb,
        .ptw_resp_r, .ptw_resp_w, .ptw_resp_x,
        .ptw_resp_u, .ptw_resp_g, .ptw_resp_a, .ptw_resp_d
    );

    // =========================================================================
    // Frontend — FIX 1: tipo alinhado
    // =========================================================================
    nebula_frontend_rvc #(
        .XLEN(XLEN), .VADDR_WIDTH(VADDR_WIDTH),
        .PADDR_WIDTH(PADDR_WIDTH), .VPN_WIDTH(VPN_WIDTH), .PPN_WIDTH(PPN_WIDTH)
    ) u_frontend (
        .clk, .rst_n,
        ...
        .frontend_out(frontend_packet), .frontend_valid,
        .fetch_pc_out(fetch_pc), .bp_prediction,
        .icache_req, .icache_addr, .icache_ready,
        .icache_resp_valid, .icache_resp_data, .icache_resp_error,
        .itlb_req, .itlb_vpn, .itlb_hit, .itlb_ppn,
        .itlb_page_fault, .itlb_access_fault,
        .fetch_exception(frontend_exc),
        .fetch_exception_cause(frontend_exc_cause),   // logic [5:0]
        .fetch_exception_value(frontend_exc_value),
    ...
    // =========================================================================
    nebula_backend_fpu #(
        .XLEN(XLEN), .FLEN(FLEN), .VADDR_WIDTH(VADDR_WIDTH),
        .PADDR_WIDTH(PADDR_WIDTH), .VPN_WIDTH(VPN_WIDTH), .PPN_WIDTH(PPN_WIDTH)
    ) u_backend (
        .clk, .rst_n,
        ...
        .dtlb_hit, .dtlb_ppn, .dtlb_page_fault,
        .ptw_req(ptw_req_dtlb), .ptw_ready,
        .ptw_resp_valid(ptw_resp_valid && !ptw_is_for_itlb), .ptw_page_fault,
        .csr_req, .csr_addr, .csr_wdata, .csr_op, .csr_rdata, .csr_fault,
        .trap_enter, .trap_is_interrupt, .trap_pc, .trap_cause, .trap_value,
        .mret_exec, .sret_exec,
        .trap_vector, .return_pc, .return_priv,
        .interrupt_pending, .interrupt_cause,
        .mdu_req, .mdu_rs1, .mdu_rs2, .mdu_funct3, .mdu_is_word,
        .mdu_ready, .mdu_resp_valid, .mdu_result,
        .fpu_req, .fpu_op, .fpu_rs1, .fpu_rs2, .fpu_rs3, .fpu_int_rs1,
